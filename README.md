...
Duoids are a lesser-known algebraic structure that relate two monoids. The most strikingly useful applications for programming involve error handling and IO.

What duoids offer there is an abstraction over the sequential (`Either` and `IO`) and parallel (`Validation` and `Concurrently`) behaviors. Since it has a coherent relationship

A duoidal category (also known as "2-monoidal categories" and "2-fold monoidal categories") is a category with two monoidal structures related by an interchange law.

A braided monoidal category is a duoidal category where the two monoidal structures coincide:

```haskell
instance BraidedMonoidalCategory c t => DuoidalCategory c t t where
  interchange = braid -- not quite
```

## references

- [duoidal category](https://ncatlab.org/nlab/show/duoidal+category) on nLab
- [Monoidal Functors, Species and Hopf Algebras](http://www.math.iitb.ac.in/~swapneel/aguiar.pdf) by Marcelo Aguiar and
  Swapneel Mahajan (called "2-Monoidal Categories")
- [United Monoids](https://blogs.ncl.ac.uk/andreymokhov/united-monoids/) by Andrey Mokhov

## usage

See the package READMEs for usage information:

- [duoids](./core/README.md) – the core implementation
- [algebraic-graph-duoids](./algebraic-graphs/README.md) – instances for [the algebraic-graphs library](https://hackage.haskell.org/package/algebraic-graphs)
- [duoidal-transformers](./transformers/README.md) – classes and instances extending [the transformers library](https://hackage.haskell.org/package/transformers)
- [duoids-hedgehog](./hedgehog/README.md) – testing your duoid instances

**NB**: This project currently supports GHC 9.6 and newer. There is no known impediment to supporting older versions. If you need support for an older version, please [open an issue](https://github.com/sellout/duoids/issues). Even if it already compiles on that version, knowing that someone is using it will allow me to add it to my support matrix so it’s likely to continue working going forward.

One immediately “neat” use cases that exercises most of the packages here something like `Writer (Graph a))`. If each new action is annotated with a unique value, say `tell (Vertex "some action")` , then the result will have a dependency graph of all actions.
...
Duoidal.do
  ...
  a1b <- 5 <$ tell (Vertex "action action 1b")
